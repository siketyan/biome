    ...
    R: Rule<Options: Default> + 'static,
{
    ...
    fn actions(&self, filter: ActionFilter) -> AnalyzerActionIter<RuleLanguage<R>> {
        ...
        let configured_applicability = if filter.is_rule_fix() && !fix_disabled {
            configured_applicability(configured_fix_kind)
        } else {
            None
        };
        let options = self.options.rule_options::<R>().unwrap_or_default();
...
/// The applicability configured for a rule's fix, if any.
fn configured_applicability(fix_kind: Option<FixKind>) -> Option<Applicability> {
    match fix_kind? {
        FixKind::None => None,
...
/// Converts a rule's fix into an [AnalyzerAction], monomorphized per language
/// instead of per rule.
...
/// Converts a rule's suppression into an [AnalyzerAction], monomorphized per
/// language instead of per rule.
...
/// Body of [AnalyzerSignal::actions_metadata] for [RuleSignal], kept out of
/// the generic method so it is compiled once instead of once per rule.
