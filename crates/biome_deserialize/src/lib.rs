...
/// ```
/// Reports a key that no field of the deserialized struct accepts.
///
/// Called from code generated by the [biome_deserialize_macros::Deserializable]
/// derive. It is kept out of line so that the diagnostic is built once instead
/// of once per derived struct.
#[cold]
#[inline(never)]
pub fn report_unknown_key(
    ctx: &mut dyn DeserializationContext,
    key: &str,
    range: TextRange,
    allowed_keys: &[&str],
    ...
    let diagnostic = DeserializationDiagnostic::new_unknown_key(key, range, allowed_keys);
    ctx.report(match severity {
        Some(severity) => diagnostic.with_custom_severity(severity),
        None => diagnostic,
    });
}

/// Reports a required key that the deserialized struct did not carry.
///
/// Called from code generated by the [biome_deserialize_macros::Deserializable]
/// derive. It is kept out of line so that the diagnostic is built once instead
/// of once per derived struct.
